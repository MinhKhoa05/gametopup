...
import { Badge, Button, Field, IconBox, RecordRow, SectionHeading, StepProgress } from '@/shared/components';
import { classNames } from '@/shared/lib/classNames';
import { formatCurrency } from '@/shared/lib/format';
import { getDepositRequestFlowStep } from '@/features/wallet/lib/deposit-request-status';
import type { DepositRequest } from '@/features/wallet/types';

const WALLET_DEPOSIT_FLOW_STEPS = [
  {
    icon: <QrCode size={16} />,
    title: 'Tạo yêu cầu',
  },
...
const QUICK_AMOUNTS = [100000, 200000, 500000, 1000000] as const;

type WalletDepositFlowPanelProps = {
  amount: string;
  busy: boolean;
...
export function WalletDepositFlowPanel({
  amount,
  busy,
  ...
  return (
    <div className="gt-surface-ink rounded-2xl p-6">
      <SectionHeading
        ...
        action={<Badge variant="accent">{formatCurrency(walletBalance)}</Badge>}
      />
      ...
      {!deposit ? (
        <div className="mt-5 grid gap-4">
          <StepProgress currentStep={1} steps={WALLET_DEPOSIT_FLOW_STEPS} />

          <form className={classNames('grid gap-4', !userDisplayName && 'pointer-events-none opacity-60')} onSubmit={onSubmit}>
              ...
              {QUICK_AMOUNTS.map((value) => (
                <button
                  ...
                  className={classNames(
                    ...
                    amount === String(value) ? 'border-cyan bg-cyan/15 text-cyan-50' : 'border-white/10 bg-white/4 text-slate-300',
                  )}
                >
            ...
            <Button className="w-full text-lg" type="submit" variant="accent" disabled={!userDisplayName || busy || createDepositPending}>
              <CreditCard size={18} />
              Tạo yêu cầu nạp
            </Button>
          </form>
        ...
        <div className="mt-5 grid gap-4">
          <StepProgress currentStep={getDepositRequestFlowStep(deposit.status)} steps={WALLET_DEPOSIT_FLOW_STEPS} />

          <div className="grid gap-4 lg:grid-cols-[minmax(0,1.03fr)_minmax(0,0.97fr)]">
            ...
            <div className="min-w-0 rounded-2xl border border-white/6 bg-white/3 p-4">
              ...
              <div className="grid gap-2">
                <RecordRow className="grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] px-4 py-3">
                  ...
                  <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-right">
                ...
                <RecordRow className="grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] px-4 py-3">
                  ...
                  <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-right">
                    <strong className="break-words font-mono text-right text-sm font-black tracking-tight text-white">{deposit.accountNo ?? '---'}</strong>
                    <CopyActionButton
                ...
                <RecordRow className="grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] px-4 py-3">
                  ...
                  <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-right">
                    <strong className="break-words text-right text-sm font-black text-white">{deposit.accountName ?? '---'}</strong>
                    <CopyActionButton
                ...
                <RecordRow highlighted className="grid-cols-[minmax(0,0.72fr)_minmax(0,1fr)] px-4 py-3">
                  ...
                  <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-2 text-right">
                    <strong className="max-w-full break-words text-right font-mono text-sm font-black tracking-tight text-cyan-50">
...
const BANK_DISPLAY_NAME_BY_ID: Record<string, string> = {
  vcb: 'Vietcombank',
  vietcombank: 'Vietcombank',
};

const BANK_LOGO_LABEL_BY_ID: Record<string, string> = {
  vcb: 'VCB',
  vietcombank: 'VCB',
};
...
function getBankDisplayName(bankId?: string): string {
  ...
  return BANK_DISPLAY_NAME_BY_ID[normalized] ?? bankId ?? 'Ngân hàng liên kết';
}

function getBankLogoLabel(bankId?: string) {
  const normalized = bankId?.trim().toLowerCase();
  if (!normalized) return 'NH';

  return BANK_LOGO_LABEL_BY_ID[normalized] ?? normalized.slice(0, 3).toUpperCase();
}
...
function BankLogoBadge({ bankId, bankName }: { bankId?: string; bankName: string }) {
  ...
  return (
    <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-1">
      ...
      <strong className="max-w-[10rem] truncate text-right text-sm font-black text-white">{bankName}</strong>
    </span>
  );
}
