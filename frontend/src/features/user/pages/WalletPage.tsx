  ...
  if (!user) {
    return (
      <EmptyState
        ...
        icon={<IconBox size="lg" className="mx-auto mb-4"><WalletCards size={24} /></IconBox>}
...
function DepositRequestList({
  ...
  return (
    <section className="rounded-2xl border border-white/6 bg-ink-light">
      ...
      {loading ? (
        <EmptyState className="border-t border-white/6 rounded-none bg-transparent px-6 py-6" title="Đang tải yêu cầu nạp..." />
      ) : recentRequests.length === 0 ? (
        <EmptyState
          className="border-t border-white/6 rounded-none bg-transparent px-6 py-6"
...
function DepositAmountForm({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-2xl border border-white/6 bg-ink-light p-6"
    >
      <SectionHeading
        className="mb-6"
        action={<IconBox size="md"><WalletCards size={22} /></IconBox>}
        title="Tạo lệnh nạp"
        description="Chọn số tiền muốn nạp vào ví của bạn."
      />
...
function WalletTransactionList({
  ...
  if (loading) {
    return <EmptyState className="border-t border-white/6 rounded-none bg-transparent px-6 py-6" title="Đang tải lịch sử ví..." />;
  }

  if (transactions.length === 0) {
    return <EmptyState className="border-t border-white/6 rounded-none bg-transparent px-6 py-6" title="Chưa có giao dịch phù hợp." />;
  }
