  ...
  return (
    <section id="packages" className="grid gap-4 scroll-mt-24">
      <div className="space-y-2">
        <h2 className="text-[clamp(1.2rem,1.8vw,1.6rem)] font-black tracking-tight text-white">Danh sách gói nạp</h2>
      ...
      {isLoading && packages.length === 0 ? (
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(170px,214px))] justify-start gap-3">
          {packages.map((pkg) => (
...
function GamePackageGridSkeleton() {
  return (
    <div className="grid grid-cols-[repeat(auto-fill,minmax(170px,214px))] justify-start gap-3" aria-busy="true" aria-label="Đang tải gói nạp">
